...
print_header() {
    ...
    echo -e "${NC}"
    echo -e "${CYAN}==============================================================${NC}"
    echo -e "${CYAN}DATASCIENTEST LABS Simulator: Kubernetes Certification Exam Simulator${NC}"
    echo -e "${CYAN}Practice in a realistic environment for CKA, CKAD, and more${NC}"
    echo -e "${CYAN}==============================================================${NC}"
    echo -e "${CYAN} Facing any issues? Please report at: https://github.com/nishanb/DATASCIENTEST LABS/issues${NC}"
    echo
}
...
main() {
    ...
    mkdir -p DATASCIENTEST LABS-simulator && cd DATASCIENTEST LABS-simulator
    
    # Download docker-compose.yml
    echo -e "${YELLOW}Downloading Docker Compose file...${NC}"
    curl -fsSL https://raw.githubusercontent.com/nishanb/DATASCIENTEST LABS/master/docker-compose.yaml -o docker-compose.yml
    ...
    # Start services
    echo -e "${YELLOW}Starting DATASCIENTEST LABS services...${NC}"
    docker compose up -d
    ...
    echo -e "${CYAN}==============================================================${NC}"
    echo -e "${GREEN}✓ DATASCIENTEST LABS Simulator has been installed successfully${NC}"
    ...
    echo -e "${CYAN}==============================================================${NC}"
    echo -e "${YELLOW}DATASCIENTEST LABS Simulator has been installed in:${NC} ${GREEN}$(pwd)${NC}, run all below commands from this directory"
    echo -e "${YELLOW}To stop DATASCIENTEST LABS  ${GREEN}docker compose down --volumes --remove-orphans --rmi all${NC}"
    echo -e "${YELLOW}To Restart DATASCIENTEST LABS:${NC} ${GREEN}docker compose restart${NC}"
    echo -e "${YELLOW}To clean up all containers and images:${NC} ${GREEN}docker system prune -a${NC}"
    echo -e "${YELLOW}To remove only DATASCIENTEST LABS images:${NC} ${GREEN}docker compose down --rmi all${NC}"
    echo -e "${YELLOW}To access DATASCIENTEST LABS Simulator:${NC} ${GREEN}https://play.sailor.sh/${NC}"
    echo
    echo -e "${CYAN}Thank you for installing DATASCIENTEST LABS Simulator!${NC}"
}
